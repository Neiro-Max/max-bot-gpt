...
def bp_contract_check_start(call):
    ...
    # Попробуем показать всплывашку на кнопке (визуальная обратная связь)
    try:
        bot.answer_callback_query(call.id, "Ок, пришлите файл договора")
    except Exception:
        pass

    # Основная отправка в текущий чат + резерв в личку на случай ошибки
    try:
        ...
        print("SEND ERR:", e)
        try:
            bot.send_message(user_id, msg)  # запасной канал
        except Exception as e2:
            print("SEND Fallback ERR:", e2)
